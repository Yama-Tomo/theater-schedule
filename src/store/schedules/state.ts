...
export const initialStateResolver = (data: any): State => {
  ...
  const schedules: Schedule[] = data.schedules.map((schedule: any) => {
    return {
      ...
      theaterSiteUrl: schedule.theater_site_url
        ? schedule.theater_site_url
        : undefined,
      runningTime: schedule.running_time ? schedule.running_time : undefined,
      recess: schedule.recess ? schedule.recess : undefined,
      ticketsOnTheDay: schedule.tickets_on_the_day
        ? schedule.tickets_on_the_day
        : undefined,
      ticketsOnTheDayInformationUrl: schedule.tickets_on_the_day_information_url
        ? schedule.tickets_on_the_day_information_url
        : undefined,
      officialSiteUrl: schedule.official_site_url
        ? schedule.official_site_url
        : undefined,
      actors: schedule.actors
        ? schedule.actors.map((actor: any) => ({
            name: actor.name,
          }))
        : [],
    };
