...
const actions = {
  fetchInitialState(ctx: Context) {
    const data = process.env.isDev
      ? require('~/data/schedules.yml')
      : require('~/data/schedules.json');
    ctx.commit('setInitialState', initialStateResolver(data));
  },
  updateSearchWord: (ctx: Context, args: string) =>
    ctx.commit('updateSearchWord', args),
};
